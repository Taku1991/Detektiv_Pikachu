...
async def setup_cogs(bot):
    ...
    for cog_dir in cog_dirs:
        ...
        logger.info(f"Durchsuche Cog-Verzeichnis: {cog_dir}")
        ...
        for cog_file in cog_files:
            ...
            try:
                logger.debug(f"Lade Cog: {module_path}")
                await load_cog(bot, module_path)
                logger.info(f"Cog geladen: {module_path}")
            except Exception as e:
                logger.error(f"Fehler beim Laden von Cog {module_path}: {e}")
